...
dataset = torch.load("D:\\scenario_gan\\dataset.pth").cpu()
...
def interval_plot(day, scenior_gen_out, area_num=0):
    ...
    y20=quantile_values[area_num, 0:96, 2]
    y30=quantile_values[area_num, 0:96, 3]
    y70=quantile_values[area_num, 0:96, 4]
    y80=quantile_values[area_num, 0:96, 5]
    y90=quantile_values[area_num, 0:96, 6]
    y100=quantile_values[area_num, 0:96, 7]
...
for day in range(300,305):
    ...
    condition=condition.reshape([1, 1, 1, 96])
    ...
    scenior_gen_out= torch.zeros(sec, 1, 1, 1, 96).type(torch.FloatTensor)
    for i in range(sec):
        noise_input=torch.randn(1, 1, 1, 96)
        gen_input=torch.cat([condition, noise_input], 2)
        gen_out = gen(gen_input.type(torch.FloatTensor)).reshape([1, 1, 1, 1, 96])
        scenior_gen_out[i, :]=gen_out.detach()
    torch.save(scenior_gen_out, r'D:\\scenario_gan\\picture\\scenior_{}_{}.pth'.format(day, sec))
    ...
    for i in range(20-1): 
        ...
        plt.tight_layout() 
    plt.plot(dataset[day, :], linestyle='-',linewidth=2, alpha=0.7, color = "blue")
    #plt.show()
    plt.savefig(r'D:\\scenario_gan\\picture\\scenior_{}_{}.png'.format(day, sec))
