"""
Donate my code for free!!!

The author's name is '''Ziyuan Song'''

What is the code:
1. The code aims to generates tomorrow's PV probability prediction based on today's 96-point PV output curve.
2. A very easy model, may not be pretty well. But with all the functions.
3. The document should be in "D:\scenario_gan"


Step1:
Run the ''D:\scenario_gan\experiments\main.py'' 

Step2:
Take your tea until the trainning finish.

Step3:
Run the ''D:\scenario_gan\CGAN_gen_model_tomorrow.py'' 

"""
