...
how_many_train = 256  # 拿前300天的数据训练用于训练，后面保留60天测试
def train_gan(dataset, params, gan_type, dis_train_steps, gen_train_steps, SAVE_DIR, dtype):
    ...
    condition=torch.zeros([how_many_train, 1, 1, 96]).type(dtype)
    reals=torch.zeros([how_many_train, 1, 1, 96]).type(dtype)
        ...
        for iteration in range(int(random_idx.shape[0]/minibatchsize)):
            ## create mini-batches
                ...
                for i in range(dis_train_steps):
                    ...
                    noise_data=torch.randn((minibatchsize, 1, 1, 96)).type(dtype)
                    ...
                    # gradient penalty
                    alpha = torch.rand(minibatchsize,1,2,96)
                    alpha = alpha.cuda()
            ...
            ## train generator (i-times per epoch):
                for i in range(gen_train_steps):            
                    gen.zero_grad()
                    gen_noise_data=torch.randn(minibatchsize, 1, 1, 96).type(dtype)
